...
PbnStream.prototype._transform = function(line, encoding, done) {
    if (this.comment) {
        if (line.endsWith('}')) {
            ...
            if (lineData) {
                this.comment.text += crlf + line.replace('}', '').trim();
            }
        ...
        // Hackfix for tags with comments among the data
        if (this.tag) {
            this.tag.section = this.tag.section || [];
            this.tag.tokens = this.tag.tokens || [];
            this.tag.section.push(line.slice(0, braceIndex - 1).replace('{', '').replace('}', '').trim());

            Array.prototype.push.apply(this.tag.tokens, line.slice(0, braceIndex - 1).replace('{', '').replace('}', '').trim().match(/\S+/g));
        }
        let rest = line.slice(braceIndex + 1).trim();
        if (rest.endsWith('}')) {
            this.comment.text += rest.replace('}', '').trim();
            this.produce(this.comment);
            this.comment = undefined;
        } else {
            this.comment.text += rest.trim();
        }
    }
    else if (this.tag) {
        this.tag.section = this.tag.section || [];
